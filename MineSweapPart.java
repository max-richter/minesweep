...
public class MineSweapPart extends JFrame
{
	...
	private class MyListener implements ActionListener
	{
		...
		public void exposeCell(MyJButton jb)
		{
			...
			if ( sGrid[jb.row][jb.col] == 9 )
			{
				JOptionPane.showMessageDialog(null, "Sorry... you just lost!\nYou found " + (MineSweapPart.MINES - MineSweapPart.actualMinesLeft) + " mines!");
				System.exit(0);
				return;
			}
	...
	private void setMines()
	{
		...
		for (int i = 0; i < MINES; i++) {
			...
			int random_col = ThreadLocalRandom.current().nextInt(0, COLS);
			
			if (!(sGrid[random_row][random_col] == 9)) {
				
				sGrid[random_row][random_col] = 9;
				
				if (random_row + 1 < sGrid.length) {
					
					if (sGrid[random_row+1][random_col] != 9) {
						
						sGrid[random_row+1][random_col]++;
						
					}
					
					if (random_col + 1 < sGrid[random_row+1].length && sGrid[random_row+1][random_col+1] != 9) {
						
						sGrid[random_row+1][random_col+1]++;
						
					}
					
					if (random_col - 1 >= 0 && sGrid[random_row+1][random_col-1] != 9) {
				...
				if (random_col + 1 < sGrid[random_row].length && sGrid[random_row][random_col+1] != 9) {
					
					sGrid[random_row][random_col+1]++;
					
				}
				
				if (random_col - 1 >= 0 && sGrid[random_row][random_col-1] != 9) {
